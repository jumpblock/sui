...
pub struct CacheUpdateHandler {
    ...
    sender:Sender<Vec<(ObjectID, Object)>>
}
...
impl CacheUpdateHandler {
    pub fn new() -> Self {
        ...
        let (sender,rx)=crossbeam_channel::unbounded::<Vec<(ObjectID, Object)>>();
        tokio::spawn(async move {
            Self::write_socket(connections, rx).await;
        });
        Self {
            socket_path,
            running,
    ...
    async fn write_socket(connections: Arc<Mutex<Vec<UnixStream>>>,rx:Receiver<Vec<(ObjectID, Object)>>){
        while let Ok(objects)=rx.recv() {
    ...
    pub fn notify_written(&self, objects: Vec<(ObjectID, Object)>) {
        self.sender.send(objects).expect("notify_written error");
    }
}
